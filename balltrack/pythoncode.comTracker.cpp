...
vector<Point> H_ROI, S_ROI, V_ROI;// HSV values from the click/drag ROI region stored in separate vectors so that we can sort them easily
	...
	if (mouseMove == false && rectangleSelected == true){
		...
		if (rectangleROI.width<1 || rectangleROI.height<1) cout << "Please drag a rectangle, not a line" << endl;
			...
			for (int i = rectangleROI.x; i < rectangleROI.x + rectangleROI.width; i++) {
				for (int j = rectangleROI.y; i < rectangleROI.y + rectangleROI.height; j++) {
					H_ROI.push_back(hsv_frame.at(j, i)[0]);
					S_ROI.push_back(hsv_frame.at(j, i)[1]);
					V_ROI.push_back(hsv_frame.at(j, i)[2]);
				}
			}
		}
